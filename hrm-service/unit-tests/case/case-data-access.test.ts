...
import { expectValuesInSql, getSqlStatement, mockConnection, mockTask, mockTransaction } from '../mock-pgpromise';
...
describe('getById', ()=>{
  ...
  test('get non existing case returns undefined', async () => {
    ...
    expect(oneOrNoneSpy).toHaveBeenCalledWith(expect.stringContaining('CSAMReports'), expect.objectContaining({ accountSid, caseId }));
...
describe('list', ()=> {
  ...
  describe('query parameters', ()=> {
    ...
    .test('$description', async ({parameters, expectedDbParameters}) => {
      const dbResult = [{ ...createMockCaseRecord({id: 2}), totalCount: 1337 }, { ...createMockCaseRecord({id: 1}), totalCount: 1337 }];
  ...
  each([
    ...
    .test('$description', async ({parameters, expectedDbParameters, dbResult, expectedResult = dbResult}) => {
      const anySpy = jest.spyOn(conn, 'any').mockResolvedValue(dbResult);
      const result = await caseDb.list(parameters, accountSid);
      expect(anySpy).toHaveBeenCalledWith(expect.stringContaining('Cases'), expect.objectContaining({...expectedDbParameters, accountSid}));
...
describe('update', ()=> {
  ...
  test('runs update SQL against cases table with provided ID.', async ()=> {
    ...
    const multiSpy = jest.spyOn(tx, 'multi').mockResolvedValue([[{ ...createMockCaseRecord({}), id: caseId}],[{ ...caseUpdateResult, id: caseId}]]);
    ...
    expect(multiSpy).toHaveBeenCalledWith(expect.any(String), expect.objectContaining({ accountSid, caseId }))
    expect(result).toStrictEqual({ ...caseUpdateResult, id: caseId});
  });

  test('creates audit record if a record was updated.', async ()=> {
    const caseUpdateResult = { ...createMockCaseRecord(caseUpdate), id: caseId, accountSid, twilioWorkerId: workerSid};
    jest.spyOn(tx, 'multi').mockResolvedValue([[{ ...createMockCaseRecord({}), id: caseId}],[caseUpdateResult]]);
  ...
  test('does not write an audit record if no record was updated.', async ()=> {
    const updateSpy = jest.spyOn(tx, 'multi').mockResolvedValue([[{ ...createMockCaseRecord({}), id: caseId}],[]]);
    const auditSpy = jest.spyOn(tx, 'none');
...
describe('delete', ()=> {
  test('returns deleted value if something at the specified ID exists to delete', async ()=>{
    ...
    expect(oneOrNoneSpy).toHaveBeenCalledWith(expect.stringContaining("Cases"), [ accountSid, caseId ]);
    expect(result).toStrictEqual(caseFromDB);
  });
