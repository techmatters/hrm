...
import {
  ...
  SELECT_CASE_LIST,
  SELECT_CASE_SEARCH, selectSingleCaseByIdSql,
  updateByIdSql,
} from './case-sql';
...
const logCaseAudit = async (transaction: pgPromise.ITask<unknown>, updated: CaseRecord, original?: Case) => {
...
export const getById = async (caseId: number, accountSid: string): Promise<CaseRecord | undefined> => {
  return db.task(
    async connection => {
...
export const list = async (query: { helpline: string}, accountSid): Promise<{ cases: readonly CaseRecord[], count: number}> => {
...
export const update = async (id, body: Partial<CaseRecord>, accountSid, workerSid): Promise<CaseRecord> => {
  ...
  const result = await db.tx(
    async transaction => {
      ...
      const [original, updated] = await transaction.multi<Case>(updateById, { accountSid, caseId: id, });
