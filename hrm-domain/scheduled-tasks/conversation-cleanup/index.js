...
exports.cleanupConversations = void 0;
...
const PAGE_SIZE = 40;
const removeConversationsBySids = async (accountSid, conversationSids, attemptToClose) => {
    const twilioClient = await (0, twilio_client_1.getClient)({ accountSid });
    ...
    const removePromises = conversationSids.map(async (sid) => {
        if (attemptToClose) {
            ...
            try {
                ...
                console.warn(`Error occurred while closing conversation ${sid} from account ${accountSid}, removing instead`, e);
            }
        }
        ...
        try {
            ...
            if (result) {
                ...
                console.warn(`Failed to remove conversation ${sid} from account ${accountSid}. Method returned false`);
            }
        }
        catch (err) {
            console.warn(`Failed to remove conversation ${sid} from account ${accountSid}. Error thrown:`, err);
        }
    });
...
const removeOldConversations = async (accountSid, retentionDays, errorIfFound = false, state) => {
    const endDate = (0, subDays_1.default)(new Date(), retentionDays);
    ...
    while (conversationsRetrieved >= PAGE_SIZE && iteration++ < 100) {
        ...
        const response = await fetch(`https://conversations.twilio.com/v1/Conversations?PageSize=${PAGE_SIZE}&EndDate=${endDate.toISOString()}${state ? `&State=${state}` : ''}`, {
            headers: {
                Authorization: `Basic ${Buffer.from(`${accountSid}:${authToken}`).toString('base64')}`,
            },
        });
        if (!response.ok) {
            throw new Error(`Error retrieving conversations from Twilio API for URL ${response.url} Status: ${response.status}: ${await response.text()}`);
        }
        ...
        if (conversationSids.length > 0) {
            if (errorIfFound) {
                console.error(`POSSIBLE STALE CONVERSATIONS DETECTED! ${conversationSids.length} conversations found older than ${retentionDays}${state ? ` in a '${state}' state` : ''} for account ${accountSid}:`, conversationSids);
            }
            await removeConversationsBySids(accountSid, conversationSids, state && state !== 'closed');
        }
    ...
    if (iteration >= MAX_ITERATIONS) {
        console.warn(`Repeated the pull for 100 conversations for cleanup for account ${accountSid} ${iteration} ${state ? `(state '${state}')` : ''} times, this could indicate an issue with the cleanup logic`);
    }
};
/**
...
const cleanupConversations = async () => {
    await (0, ssm_cache_1.loadSsmCache)({
        configs: [
            {
    ...
    const accountSids = Object.values((0, ssm_cache_1.getCachedParameters)(/^\/[^\/]+\/twilio\/[^\/]+\/account_sid$/));
    ...
    for (const accountSid of accountSids) {
        ...
        await removeOldConversations(accountSid, MAX_ACTIVE_CONVERSATION_HOURS, true, 'active');
        await removeOldConversations(accountSid, MAX_ACTIVE_CONVERSATION_HOURS, true, 'inactive');
...
exports.cleanupConversations = cleanupConversations;
