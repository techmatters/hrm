...
const contactFieldExclusions_1 = require("../../contact/contactFieldExclusions");
...
const createMockRules = (editContactFieldConditions) => {
    return {
        viewContact: [],
        editContact: [],
...
describe('getExcludedFields', () => {
    describe('Global condition sets (no field conditions)', () => {
        it('should allow all fields when isOwner condition evaluates true', async () => {
            ...
            const user = (0, twilio_worker_auth_1.newTwilioUser)(accountSid, workerSid, []);
            const getExcluded = (0, contactFieldExclusions_1.getExcludedFields)(rules);
        ...
        it('should allow all fields when everyone condition is present', async () => {
            ...
            const user = (0, twilio_worker_auth_1.newTwilioUser)(accountSid, anotherWorkerSid, []);
            const getExcluded = (0, contactFieldExclusions_1.getExcludedFields)(rules);
            const excludedFields = await getExcluded(mockContact, user, 'editContactField');
            expect(excludedFields).toEqual({});
        });
        it('should allow all fields when isSupervisor condition evaluates true', async () => {
            ...
            const user = (0, twilio_worker_auth_1.newTwilioUser)(accountSid, anotherWorkerSid, ['supervisor']);
            const getExcluded = (0, contactFieldExclusions_1.getExcludedFields)(rules);
            const excludedFields = await getExcluded(mockContact, user, 'editContactField');
            expect(excludedFields).toEqual({});
        });
    ...
    describe('Field-specific condition sets', () => {
        ...
        it('should allow field when field-specific condition evaluates true', async () => {
            ...
            const user = (0, twilio_worker_auth_1.newTwilioUser)(accountSid, workerSid, ['supervisor']);
            const getExcluded = (0, contactFieldExclusions_1.getExcludedFields)(rules);
            const excludedFields = await getExcluded(mockContact, user, 'editContactField');
            expect(excludedFields).toEqual({});
        });
    ...
    describe('System user', () => {
        it('should allow all fields for system users', async () => {
            ...
            const user = (0, twilio_worker_auth_1.newAccountSystemUser)(accountSid);
            const getExcluded = (0, contactFieldExclusions_1.getExcludedFields)(rules);
            const excludedFields = await getExcluded(mockContact, user, 'editContactField');
            expect(excludedFields).toEqual({});
        });
