...
const listPermissionWhereClause = ({ conditionWhereClauses, listConditionSets, user, }) => {
    const ALL_OR_NOTHING_CONDITIONS = ['everyone', 'isSupervisor'];
    const conditionSetClauses = [];
    const conditionsThatAllowAll = user.isSupervisor
        ? ALL_OR_NOTHING_CONDITIONS
        : ['everyone'];
    const conditionsThatBlockAll = user.isSupervisor
        ? []
        : ['isSupervisor'];
    for (const caseListConditionSet of listConditionSets) {
        ...
        if (relevantConditions.length) {
            const conditionClauses = relevantConditions.map(condition => {
    ...
    return conditionSetClauses.length ? conditionSetClauses : hrm_search_config_1.FILTER_ALL_CLAUSE;
};
