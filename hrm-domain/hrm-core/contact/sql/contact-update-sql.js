...
exports.UPDATE_CASEID_BY_ID = exports.generateUpdateContactByIdSql = void 0;
const contact_get_sql_1 = require("./contact-get-sql");
const ID_WHERE_CLAUSE = `WHERE "accountSid" = $<accountSid> AND "id"=$<contactId>`;
const generatePartialFormUpdateClause = (formName, excluded = []) => {
    if (excluded.length) {
        const exclusionObject = `jsonb_build_object(${excluded.map(excludedField => `'${excludedField}', "rawJson"->'${formName}'->'${excludedField}'`)})`;
...
const generateUpdateContactByIdSql = (fieldExclusions) => `WITH updated AS (
UPDATE "Contacts"
SET "rawJson" = COALESCE("rawJson", '{}'::JSONB)
  || ${generatePartialFormUpdateClause('caseInformation', fieldExclusions.caseInformation)}
  || (CASE WHEN $<categories> IS NOT NULL THEN jsonb_build_object('categories', $<categories>::JSONB) ELSE '{}'::JSONB END)
  || ${generatePartialFormUpdateClause('callerInformation', fieldExclusions.callerInformation)}
  || ${generatePartialFormUpdateClause('childInformation', fieldExclusions.childInformation)}
  || ${generatePartialFormUpdateClause('contactlessTask', fieldExclusions.contactlessTask)}
...
${(0, contact_get_sql_1.selectSingleContactByIdSql)('updated')}
`;
exports.generateUpdateContactByIdSql = generateUpdateContactByIdSql;
exports.UPDATE_CASEID_BY_ID = `WITH updated AS (
UPDATE "Contacts"
SET
  "caseId" = $<caseId>,
  "updatedAt" = CURRENT_TIMESTAMP,  
