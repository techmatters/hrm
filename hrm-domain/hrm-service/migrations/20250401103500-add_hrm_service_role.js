...
module.exports = {
  up: async queryInterface => {
    ...
    await queryInterface.sequelize.query(`GRANT CONNECT ON DATABASE hrm TO hrm_service`);
    ...
    await queryInterface.sequelize.query(`ALTER DEFAULT PRIVILEGES IN SCHEMA public
    GRANT INSERT, UPDATE, DELETE, SELECT ON TABLES TO hrm_service`);
    console.log(
      'Added default SELECT, INSERT, UPDATE and CREATE access to tables in the public schema to hrm_service role, so it has access to all new tables',
    );
    await queryInterface.sequelize.query(`ALTER DEFAULT PRIVILEGES  IN SCHEMA public
    GRANT USAGE, SELECT, UPDATE ON SEQUENCES TO hrm_service`);
    console.log(
