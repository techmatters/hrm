...
exports.describe = 'Create an identifier (if not exists) and a new profile associated to it';
exports.builder = {
    e: {
        alias: 'environment',
...
const handler = async ({ region, environment, accountSid, identifier, name, definitionVersion, }) => {
    try {
        ...
        const url = (0, hrmInternalConfig_1.getAdminV0URL)(internalResourcesUrl, accountSid, '/profiles/identifiers');
        ...
        console.error(`Failed to create identifier ${identifier} account ${accountSid} (${region} ${environment})`, err instanceof Error ? err.message : String(err));
    }
};
