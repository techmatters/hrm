...
exports.describe = 'List the profile flags for the given account';
exports.builder = {
    e: {
        alias: 'environment',
...
const handler = async ({ region, environment, accountSid }) => {
    try {
        ...
        const url = (0, hrmInternalConfig_1.getAdminV0URL)(internalResourcesUrl, accountSid, '/profiles/flags');
        ...
        console.error(`Failed to list profile flags for account ${accountSid} (${region} ${environment})`, err instanceof Error ? err.message : String(err));
    }
};
