...
const seederDirectory = pathLib.join(process.cwd(), './seeders/');
...
const umzug = new Umzug({
  migrations: fs
    .readdirSync(pathLib.join(process.cwd(), './seeders/'))
    .filter(file => file.endsWith('.js'))
    .map(filename => {
      // eslint-disable-next-line global-require,import/no-dynamic-require
      const migration = require(pathLib.join(seederDirectory, filename));
      return {
        name: filename,
...
async function seed() {
  ...
  let ret;
  let lastErr;
  console.log('Umzug seeder starting.', pathLib.join(process.cwd(), './seeders/*.js'));
  while (Date.now() < timeoutPoint) {
    try {
      // eslint-disable-next-line no-await-in-loop
      ret = await umzug.up();
      console.log('Seed complete.', JSON.stringify(ret));
      break;
    } catch (err) {
      console.log('Seed failed. Retrying...', err);
